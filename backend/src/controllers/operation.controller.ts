...
export const operationController = {
  ...
  async getByDate(req: Request, res: Response) {
    try {
      ...
      const operations = await operationService.getOperationsByDate(date);
      res.json(operations);
    } catch (error) {
  ...
  async getByDateRange(req: Request, res: Response) {
    try {
      ...
      const operations = await operationService.getOperationsByDateRange(
        startDate as string,
        endDate as string
      );
      res.json(operations);
  ...
  async delete(req: Request, res: Response) {
    try {
      const id = typeof req.params.id === 'string' ? req.params.id : req.params.id[0];
      const success = await operationService.deleteOperation(id);
      if (!success) {
        return res.status(404).json({ error: 'Operation not found' });
      }
  ...
  async deleteByDate(req: Request, res: Response) {
    try {
      const date = typeof req.params.date === 'string' ? req.params.date : req.params.date[0];
      const count = await operationService.deleteOperationsByDate(date);
  ...
  async getDailyStats(req: Request, res: Response) {
    try {
      const date = typeof req.params.date === 'string' ? req.params.date : req.params.date[0];
      const stats = await operationService.getDailyStats(date);
      res.json(stats);
    } catch (error) {
  ...
  async getMonthlyStats(req: Request, res: Response) {
    try {
      ...
      const stats = await operationService.getMonthlyStats(
  ...
  async getByStrategyId(req: Request, res: Response) {
    try {
      const strategyId = typeof req.params.strategyId === 'string' ? req.params.strategyId : req.params.strategyId[0];
      const operations = await operationService.getOperationsByStrategyId(strategyId);
      res.json(operations);
    } catch (error) {
